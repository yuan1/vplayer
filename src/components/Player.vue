<template>
  ...
  >
    ...
    <span
      ...
      class="custom-video_play custom-video_play-pause"
      @click="pause('btn')"
    >
      pause
    </span>
    <span
      v-else
      class="custom-video_play custom-video_play-play"
      @click="play('btn')"
    >
      play
    </span>
    ...
    <transition name="fade">
      ...
      >
        ...
        <div class="custom-video_control-voice">
          <span class="custom-video_control-voice-play">vol</span>
          <div
        ...
        <!-- 全屏缩放 -->
        <span class="custom-video_control-full" @click="handleScreen"
          >full</span
        >
      </div>
    </transition>
...
export default {
  ...
  mounted() {
    ...
    this.videoDom.volume = this.playerOptions.volume / 100; // 设置初始化声音
  },
  methods: {
    initMedaData() {
      this.videoDom.preload = this.playerOptions.preload || true;
      this.videoDom.autoplay = this.playerOptions.autoplay || false;
      this.videoDom.poster = this.playerOptions.poster || "";
      ...
      if (this.playerOptions.crossOrigin) {
        this.videoDom.crossOrigin = this.playerOptions.crossOrigin;
      }
      if (
        ...
        try {
          ...
          this.videoDom.src = this.playerOptions.src;
        }
      } else {
        this.videoDom.src = this.playerOptions.src;
      }
    ...
    handleVolPrograssMove(ev) {
      ...
      this.playerOptions.volume = Math.round(this.videoDom.volume * 100);
    },
    handleVolPrograssUp() {
      // 监听声音鼠标离开事件
    ...
    fullScreen() {
      ...
      if (u.indexOf("Android") > -1 || u.indexOf("Linux") > -1) {
        ...
        this.$refs["custom-video_container"].classList.add("andrioid-full");
        this.$refs["custom-video_container"].style.width = height + "px";
        this.$refs["custom-video_container"].style.height = width + "px";
        return;
      }
    ...
    exitFullscreen() {
      ...
      if (u.indexOf("Android") > -1 || u.indexOf("Linux") > -1) {
        ...
        let height = document.documentElement.clientHeight;
        console.log(width, height);
        this.$refs["custom-video_container"].classList.remove("andrioid-full");
        this.$refs["custom-video_container"].style.width = null;
        this.$refs["custom-video_container"].style.height = null;
        return;
      }
      let de = document;
...
.custom-video_container.andrioid-full {
